...
class Javava_Base_Proto:
    ...
    def packheader(self):
        headerdata=b''.join([struct.pack('>B',self.proto_header[0]),struct.pack('>B',self.proto_header[1]),self.proto_name,self.version_number,self.message_name])
        return headerdata
...
class Javava_Unpack(Javava_Base_Proto):
    def stripheader(self,data):
        return data[len(self.packheader()):]
